...
INTERFACE = "en0"          
CAPTURE_DURATION = 15      
OUTPUT_DIR = Path("final_dataset/premium_playlist")  # <--- CHANGED FOLDER
SAMPLES_TO_COLLECT = 71   
...
class PlaylistCapture:
    ...
    def capture_loop(self):
        ...
        while count < SAMPLES_TO_COLLECT:
            ...
            print(f"\n[Premium Session {count+1}/{SAMPLES_TO_COLLECT}]")
            
            # 2. Single Load
            self.driver.get(song_url)
            
            # 3. Wait for UI and Buffer
            time.sleep(9)
            ...
            with open(self.output_dir / f"premium_{int(time.time())}.pkl", "wb") as f:
